...
export function EventsAdmin({ isLight, active, onOpenEvent }) {
  ...
  const [usePasteHtml, setUsePasteHtml] = useState(false);
  ...
  const onCreate = useCallback(async () => {
    ...
    if (usePasteHtml && !html) {
      setCreateError("Paste the event page HTML when using manual import.");
      return;
    }
    setCreating(true);
    setCreateError(null);
    try {
      const token = await user.getIdToken();
      /** @type {{ event_url: string, day_count: number, page_html?: string }} */
      const payload = { event_url: url, day_count: Number(dayCount) };
      if (usePasteHtml && html) payload.page_html = html;
      const res = await fetch("/api/admin/events", {
        method: "POST",
        headers: {
  ...
  }, [user, creating, eventUrl, dayCount, usePasteHtml, pageHtml]);
    ...
    <div className="flex w-full flex-1 flex-col gap-4 px-1 py-2 sm:px-2">
      ...
      {modalOpen
        ? createPortal(
            ...
            >
              <div
                className={`w-full max-w-lg rounded-xl p-5 ${panel}`}
              ...
              >
                ...
                <p className="mt-2 text-[0.85rem] text-[#f4f0fa]/70">
                  Paste the FabTCG organised-play event URL. Coverage days are scraped from the event page. If the
                  server gets blocked (403), paste the page HTML from your browser instead.
                </p>
                ...
                <label className="mt-4 flex items-start gap-2 text-[0.85rem] text-[#f4f0fa]/80">
                  <input
                    type="checkbox"
                    className="mt-1"
                    checked={usePasteHtml}
                    disabled={creating}
                    onChange={(e) => setUsePasteHtml(e.target.checked)}
                  />
                  <span>
                    Paste page HTML manually (View Source on the FabTCG event page, copy all, paste below)
                  </span>
                </label>
                {usePasteHtml ? (
                  <label className="mt-3 flex flex-col gap-1.5">
                    ...
                    <textarea
                      className="min-h-[10rem] w-full rounded-lg border border-white/[0.22] bg-black/35 px-3 py-2 font-mono text-[0.72rem] leading-relaxed text-[#f4f0fa] outline-none focus:border-purple-400/55"
                      placeholder="Paste full HTML from fabtcg.com event page…"
                      value={pageHtml}
                      disabled={creating}
